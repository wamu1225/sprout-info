...
// 3) 内部リンク [/sprout-info/PATH/] の PATH が実在する記事ID（または about/privacy）であること
import fs from 'fs';
...
for (const { id, block } of blocks) {
  ...
  if (content.length < 1000) errors.push(`[${id}] content が短すぎる（${content.length}字・最低1000。薄いページ対策で下限を800→1000に引き上げ 2026-07-07）`);
...
console.log(`✓ validate-data: ${ids.length} articles (${ids.join(', ')}) + link/render-smoke OK`);
