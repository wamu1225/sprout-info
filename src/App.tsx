...
import { ArrowLeft, List, ChevronRight, Menu, X, Calendar, Check, Droplets } from 'lucide-react';
...
function LeafMark() {
  return (
    <svg className="brand-logo" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <rect width="64" height="64" rx="16" fill="#eaf3e2" />
      <path d="M32 52 V30" fill="none" stroke="#3f7d34" strokeWidth="4" strokeLinecap="round" />
      <path d="M32 34 C22 34 15 28 15 19 C25 19 32 25 32 34 Z" fill="#5aa64a" />
      <path d="M32 30 C42 30 49 24 49 15 C39 15 32 21 32 30 Z" fill="#77c05f" />
      <circle cx="44" cy="44" r="4.5" fill="#8fd0e6" />
    </svg>
  );
}
...
function renderTokens(tokens: InlineToken[]): ReactNode[] {
  return tokens.map((tok, i) => {
    ...
    const href = tok.href;
    const isInternal = href.startsWith(BASE + '/') || href.startsWith('/sprout-info/');
    if (isInternal) {
      return (
        ...
        <nav className={`site-nav ${navOpen ? 'open' : ''}`} aria-label="メインナビゲーション">
          {articles.map((a) => (
            <a key={a.id} href={`${BASE}/${a.id}/`} onClick={(e) => { e.preventDefault(); navigateTo(`/${a.id}/`); setNavOpen(false); }}>
              <span className="nav-emoji"><SectionIcon name={a.icon} size={17} /></span>
...
// トップの中心体験①：Day 1→7 の栽培ログ・ステッパー
function GrowStepper() {
  return (
    <section className="grow" aria-label="約1週間の栽培の手順">
      <div className="grow-head">
        <h1 className="grow-title">約1週間で、新芽を育てる</h1>
        <p className="grow-sub">ブロッコリースプラウトを、種まきから収穫まで日ごとに。水と容器があれば台所のそばで育てられます。</p>
      </div>
      <ol className="stepper">
        {GROW_STEPS.map((s, i) => (
          <li key={i} className="step">
            <div className="step-chip">{s.when}</div>
      ...
      <p className="grow-foot">
        日数や温度は目安で、種類や室温で前後します。くわしくは <a href={`${BASE}/grow/`} onClick={(e) => { e.preventDefault(); navigateTo('/grow/'); }}>栽培の手順</a> へ。
      </p>
    </section>
  );
}

// トップの中心体験②：根毛？カビ？判別カード
function DiscernCard() {
  return (
    <section className="discern" aria-label="根毛とカビの見分け">
      <h2 className="discern-h">白いふわふわ、根毛？カビ？</h2>
      <p className="discern-lead">根元に出る白い綿の多くは、水を吸うための根毛で、正常なものです。霧吹きで見分けられます。</p>
      <div className="discern-cols">
        <div className="discern-col discern-ok">
          <div className="discern-col-head"><Check size={18} aria-hidden="true" /> {DISCERN_SUMMARY.hair.label}</div>
          <div className="discern-col-hint">{DISCERN_SUMMARY.hair.hint}</div>
        </div>
        <div className="discern-col discern-ng">
          <div className="discern-col-head"><X size={18} aria-hidden="true" /> {DISCERN_SUMMARY.mold.label}</div>
          <div className="discern-col-hint">{DISCERN_SUMMARY.mold.hint}</div>
        </div>
      </div>
      <div className="discern-table-wrap">
        <table className="discern-table">
          <thead><tr><th></th><th>根毛（正常）</th><th>カビ（異常）</th></tr></thead>
          <tbody>
            {DISCERN_ROWS.map((r, i) => (
              <tr key={i}><th scope="row">{r.aspect}</th><td>{r.hair}</td><td>{r.mold}</td></tr>
            ))}
          </tbody>
        </table>
      </div>
      <p className="discern-foot">
        <Droplets size={14} aria-hidden="true" /> 霧吹きで水を吹くと、根毛は寝て消え、カビははじいて残ります。くわしくは <a href={`${BASE}/discern/`} onClick={(e) => { e.preventDefault(); navigateTo('/discern/'); }}>根毛とカビの見分け</a> へ。
      </p>
    </section>
  );
}
...
function Home() {
  useEffect(() => { document.title = `${SITE_NAME}｜約1週間で育てる手順と根毛・カビの見分け`; window.scrollTo(0, 0); }, []);
  return (
    <>
      <GrowStepper />
      <DiscernCard />
              ...
              {g.ids.map((id) => {
                ...
                return (
                  <a key={id} href={`${BASE}/${id}/`} className="home-nav-link" onClick={(e) => { e.preventDefault(); navigateTo(`/${id}/`); }}>
                    <span className="home-nav-ico" aria-hidden="true"><SectionIcon name={a.icon} size={20} /></span>
      ...
      <div className="home-trust">
        <h2>このサイトの方針</h2>
        <ul>
          <li><strong>公式情報で確かめる</strong>：栽培は種苗メーカーの育て方、衛生は農林水産省の指針にもとづきます。</li>
          <li><strong>健康効果はうたいません</strong>：成分の効果は研究途上のものが多く、数値や倍率、特定の商品を宣伝しません。</li>
          <li><strong>安全を第一に</strong>：生食のスプラウトは食中毒のリスクがあります。においや変色があれば食べないでください。</li>
        </ul>
      </div>
    </>
  );
    ...
    <>
      ...
      <article className="section-page">
        <header className="article-header">
          <div className="article-emoji" aria-hidden="true"><SectionIcon name={article.icon} size={30} /></div>
          <h1>{article.title}</h1>
...
const ABOUT_CONTENT = `本サイト「${SITE_NAME}」は、ブロッコリースプラウトを中心に、家庭でスプラウトを安全に育てて食べるための情報をまとめたものである。トップでは約1週間の栽培の手順と、根毛とカビの見分けを示し、各ページで育て方、衛生、選び方や調理までを扱う。

## 編集と制作の方針

本サイトの内容は、農林水産省の衛生管理の指針や、種苗メーカーの家庭向けの育て方などの公開情報を参照し、事実を確認したうえで、運営者が自分の言葉で書いている。出典の文章をそのまま転載することはない。

## 健康と安全について

本サイトは、特定の健康効果や病気の予防をうたわない。成分の効果には研究途上のものが多く、数値や倍率、特定の商品を宣伝することもしない。また、生で食べるスプラウトには食中毒のリスクがあるため、衛生については公的機関の情報にもとづいて記述している。においや変色があるものは食べないでほしい。
  ...
  return (
    <footer className="site-footer">
      ...
      <div className="site-footer-note">
        本サイトはスプラウトの栽培と安全な食べ方を、農林水産省や種苗メーカーの公開情報をもとに自分の言葉でまとめたものです。健康効果はうたいません。栽培の日数や温度は目安です。
      </div>
    </footer>
  );
}
